#!/usr/bin/env python3
"""
BGRPO for single variable model with validation evaluation and plotting
Keeps the same core training logic as grpo_ablation.py but adds:
- Periodic validation evaluation during training
- Real-time plotting of validation accuracy
...
# Add argument parser - EXACTLY THE SAME AS ORIGINAL
def parse_args():
    ...
    # NEW: Add validation-specific arguments
    ...
    parser.add_argument("--wandb_project", type=str, default="bgrpo-polynomial-decomposition",
...
# --- Load Model and Tokenizer (MODIFIED TO SUPPORT KV-CACHE) ---
try:
    model, tokenizer = load_model_and_tokenizer(
        config_path=str(config_path),
        model_dir_path=str(model_dir_path),
        device=device,
        wrap_for_grpo=False,  # Same as original
        model_name=args.model_name,
        use_kvcache=args.use_kvcache  # Enable KV-cache if requested
    )
...
# ========== NEW: Validation Plotting Class ==========
class ValidationPlotter:
    """Handles real-time plotting of validation metrics"""
    ...
    def __init__(self, output_dir, plot_interval=1):
        ...
        self.ax.set_title('BGRPO Training Progress - Validation Accuracy')
        self.ax.grid(True, alpha=0.3)
        self.ax.set_ylim(0, 100)
        
    def update(self, step, val_accuracy):
        """Update the plot with new data"""
        self.steps.append(step)
        self.val_accuracies.append(val_accuracy * 100)  # Convert to percentage
        
        # Update plot every plot_interval evaluations
        if len(self.val_accuracies) % self.plot_interval == 0:
            self.refresh_plot()
    
    def refresh_plot(self):
        """Redraw the plot with current data"""
        self.ax.clear()
        self.ax.plot(self.steps, self.val_accuracies, 'b-o', linewidth=2, markersize=8)
        ...
        if self.val_accuracies:
            ...
            self.ax.axhline(y=self.val_accuracies[best_idx], color='r', linestyle='--', alpha=0.5)
            self.ax.text(self.steps[-1], self.val_accuracies[best_idx], 
                        f'Best: {self.val_accuracies[best_idx]:.1f}%', 
                        ha='right', va='bottom')
        
        # Save the plot
        plot_path = os.path.join(self.output_dir, 'validation_progress.png')
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        print(f"📊 Updated plot saved to {plot_path}")

# ========== NEW: Validation Callback ==========
class ValidationCallback(TrainerCallback):
    """Custom callback to perform validation during training"""
    
    def __init__(self, val_dataset, tokenizer, eval_steps, eval_samples, 
                 beam_width, device="cuda", plotter=None):
    ...
    def evaluate_model(self, model):
        """Evaluate model on validation set using beam search"""
        model.eval()
        ...
        correct = 0
        total = min(self.eval_samples, len(self.val_dataset['prompt']))
        
        print(f"\n🔍 Running validation on {total} samples...")
            ...
            for i in range(total):
                ...
                try:
                    pred_str, correct_beam_rank = LLM_BeamSearch_check(
                        base_model, input_str, self.tokenizer, self.device, args_beam
                    )
                    
                    if correct_beam_rank != -1 and correct_beam_rank <= self.beam_width:
                        correct += 1
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if state.global_step > 0 and state.global_step % self.eval_steps == 0:
            ...
            # Store trainer reference if not already stored
            if trainer and self.trainer is None:
                self.trainer = trainer
            
            if model:
                # Evaluate
                accuracy, correct, total, eval_time = self.evaluate_model(model)
                ...
                if accuracy > self.best_accuracy:
                    ...
                    if self.trainer:
                        ...
                        best_model_path = os.path.join(self.trainer.args.output_dir, "best_model")
                ...
                # Update plot
                if self.plotter:
                    self.plotter.update(state.global_step, accuracy)
            else:
                print("⚠️  Could not access model for validation")
        
        return control

# ========== ORIGINAL FUNCTIONS (parse_prefix_to_sympy, etc.) ==========
# [Include all the original helper functions here - parse_prefix_to_sympy, count_expression_leaves, 
#  is_valid_expression_sympy, is_valid_expression, create_symbolic_expression_prompts, 
#  reward_function, create_training_dataset]

# Copy all helper functions from grpo_ablation.py
...
if not args.disable_wandb:
    # Get sweep config from wandb if available
    wandb.init(
    ...
    sweep_run_id = wandb.run.id if wandb.run else "local"
    args.output_dir = f"{args.output_dir}/run_{sweep_run_id}_{timestamp}"

# ========== GRPO Configuration (SAME AS ORIGINAL) ==========
...
if args.val_dataset_path:
    ...
    # Default to validation_dataset.txt in same directory
    val_dataset_path = dataset_dir_path.parent / "validation_dataset.txt"
...
if val_dataset_path and val_dataset_path.exists():
    ...
    print("⚠️  No validation dataset found, skipping validation")
...
val_dataset = None
if val_dataset_path and val_dataset_path.exists():
    val_dataset = create_training_dataset(1000, val_dataset_path)  # Load up to 1000 validation samples
...
# Monkey patch the _get_train_sampler method to handle the compatibility issue
...
def patched_get_train_sampler(self, dataset=None):
    ...
    # Just call the original method without the dataset parameter
...
if val_dataset:
    ...
    print("GRPO trainer initialized with validation callback")
    print(f"Validation will run every {args.eval_steps} steps on {args.eval_samples} samples")
    print(f"Plots will be updated every {args.plot_interval} validations")
else:
...
if val_dataset:
    print("\n🎯 Initial validation...")
    initial_accuracy, correct, total, eval_time = validation_callback.evaluate_model(model)
    print(f"Initial validation accuracy: {initial_accuracy:.2%} ({correct}/{total})")
    if plotter:
        plotter.update(0, initial_accuracy)
...
if val_dataset and validation_callback.validation_history:
    ...
    import json
    val_history_path = os.path.join(args.output_dir, 'validation_history.json')
    ...
    if plotter:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        final_plot_path = os.path.join(args.output_dir, f'validation_final_{timestamp}.png')
        plt.savefig(final_plot_path, dpi=300, bbox_inches='tight')
        print(f"📊 Final plot saved to {final_plot_path}")
    
    # Log final best accuracy to WandB
    if not args.disable_wandb and validation_callback.best_accuracy > 0:
        wandb.log({"final_best_validation_accuracy": validation_callback.best_accuracy})
        wandb.summary["best_validation_accuracy"] = validation_callback.best_accuracy
        print(f"📊 Final best validation accuracy logged to WandB: {validation_callback.best_accuracy:.2%}")

# Finish WandB run
if not args.disable_wandb:
    wandb.finish()

print("\n✅ GRPO training with validation completed!")