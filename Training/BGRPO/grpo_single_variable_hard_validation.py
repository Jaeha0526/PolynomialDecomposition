...
def parse_args():
    ...
    parser.add_argument("--multisample_n", type=int, default=5,
                        help="Number of samples for multisample inference")
...
class ValidationPlotter:
    """Handles real-time plotting of validation metrics including multisample"""
    ...
    def __init__(self, output_dir, plot_interval=1):
        ...
        # Data storage
        self.steps = []
        self.beam_accuracies = []
        self.multisample_accuracies = []
        self.multisample_oracle_accuracies = []
        
    def update(self, step, beam_acc, multisample_acc, oracle_acc):
        """Update the plot with new data"""
        self.steps.append(step)
        self.beam_accuracies.append(beam_acc * 100)
        self.multisample_accuracies.append(multisample_acc * 100)
        self.multisample_oracle_accuracies.append(oracle_acc * 100)
    ...
    def refresh_plot(self):
        ...
        self.ax1.plot(self.steps, self.beam_accuracies, 'b-o', linewidth=2, markersize=8, label='Beam Search')
        ...
        if self.beam_accuracies:
            best_idx = np.argmax(self.beam_accuracies)
            self.ax1.axhline(y=self.beam_accuracies[best_idx], color='r', linestyle='--', alpha=0.5)
            self.ax1.text(self.steps[-1], self.beam_accuracies[best_idx], 
                        f'Best: {self.beam_accuracies[best_idx]:.1f}%', 
                        ha='right', va='bottom')
        
        # Plot 2: Multisample accuracies
        self.ax2.plot(self.steps, self.multisample_accuracies, 'g-s', linewidth=2, markersize=8, 
                     label='Multisample (top-1)')
        self.ax2.plot(self.steps, self.multisample_oracle_accuracies, 'r-^', linewidth=2, markersize=8, 
                     label='Multisample (oracle)')
        self.ax2.set_xlabel('Training Step')
        self.ax2.set_ylabel('Accuracy (%)')
        self.ax2.set_title('Multisample Inference Performance')
        self.ax2.grid(True, alpha=0.3)
        self.ax2.set_ylim(0, 100)
        self.ax2.legend()
        
        # Save the plot
        plot_path = os.path.join(self.output_dir, 'validation_progress_hard.png')
        plt.tight_layout()
...
class HardValidationCallback(TrainerCallback):
    ...
    def evaluate_model(self, model):
        """Evaluate model using both beam search and multisample inference"""
        model.eval()
        base_model = model.pretrained_model if hasattr(model, 'pretrained_model') else model
        ...
        print(f"\n🔍 Running hard validation on {total} samples from test_dataset_4_4...")
            ...
            for i in range(total):
                ...
                try:
                    ...
                    if correct_beam_rank != -1 and correct_beam_rank <= self.beam_width:
                        beam_correct += 1
                        oracle_correct += 1  # If beam search found it, oracle would too
                ...
                try:
                    ...
                    if found_correct:
                        ...
                        if correct_beam_rank == -1 or correct_beam_rank > self.beam_width:
                ...
                # Progress update
                if (i + 1) % 10 == 0:
                    print(f"  Progress: {i+1}/{total} - Beam: {beam_correct/(i+1):.2%}, "
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if state.global_step > 0 and state.global_step % self.eval_steps == 0:
            ...
            if model:
                ...
                print(f"✅ Beam search accuracy: {results['beam_accuracy']:.2%} ({results['beam_correct']}/{results['total']})")
                ...
                if self.trainer and self.trainer.args.report_to != []:
                    wandb.log({
                        "hard_validation/beam_accuracy": results['beam_accuracy'],
...
if val_dataset:
    ...
    print(f"Initial beam accuracy: {initial_results['beam_accuracy']:.2%}")
    print(f"Initial multisample accuracy: {initial_results['multisample_accuracy']:.2%}")
    print(f"Initial oracle accuracy: {initial_results['oracle_accuracy']:.2%}")
    if plotter:
        plotter.update(0, initial_results['beam_accuracy'], 
...
if val_dataset and validation_callback.validation_history:
    ...
    if plotter:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        final_plot_path = os.path.join(args.output_dir, f'hard_validation_final_{timestamp}.png')
        plt.savefig(final_plot_path, dpi=300, bbox_inches='tight')
        print(f"📊 Final plot saved to {final_plot_path}")
    
    # Log final best accuracies to WandB
    if not args.disable_wandb:
        wandb.log({"final_best_beam_accuracy": validation_callback.best_beam_accuracy})
        wandb.log({"final_best_multisample_accuracy": validation_callback.best_multisample_accuracy})
        wandb.summary["best_beam_accuracy"] = validation_callback.best_beam_accuracy
        wandb.summary["best_multisample_accuracy"] = validation_callback.best_multisample_accuracy
        print(f"📊 Final best accuracies logged to WandB")
