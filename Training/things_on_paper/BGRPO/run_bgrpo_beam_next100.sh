...
# Run beam eval ONLY on problems 100-199 (the "next 100" after the first 100).
# Used to extend existing 100-problem beam summaries to 200-problem coverage
# without re-running the already-completed first 100 problems.
#
# Usage:
#   sbatch run_bgrpo_beam_next100.sh <run_tag> <ckpt_name> [beam_width]
#
# Writes:
#   runs/<run_tag>/eval/<ckpt_name>/beam_next100_predictions.txt
#   runs/<run_tag>/eval/<ckpt_name>/beam_next100.out
#   runs/<run_tag>/eval/<ckpt_name>/summary_next100.json   (beam-only)
#
# Dataset:
#   data_storage/things_on_paper/dataset/d2_test_100to199/test_dataset.txt (100 lines)
...
DATASET_FILE="${REPO}/data_storage/things_on_paper/dataset/d2_test_100to199/test_dataset.txt"
...
echo "=== BGRPO beam-next100 eval: run=${RUN_TAG} ckpt=${CKPT_NAME} ==="
echo "  ckpt:    $CKPT_PATH"
echo "  dataset: $DATASET_FILE  (100 problems, indices 100-199)"
...
python3 "${REPO}/Training/mingpt/run.py" debug_beam \
    ...
    --outputs_path "${OUT_DIR}/beam_next100_predictions.txt" \
    --extended_vocab --sympy 1 \
    2>&1 | tee "${OUT_DIR}/beam_next100.out"
...
summary = {"run_tag": out_dir.parent.name, "ckpt": out_dir.name, "beam_width": bw,
           "split": "next100_problems_100_to_199"}
b_text = (out_dir / "beam_next100.out").read_text(errors="ignore")
beam_by_width = {}
...
(out_dir / "summary_next100.json").write_text(json.dumps(summary, indent=2))
print(f"\nwrote {out_dir / 'summary_next100.json'}")
PY
