...
# Generic beam-search evaluation wrapper.
#
# Usage:
#   sbatch eval/run_beam_eval.sh configs/<name>.env [split] [beam_width]
#
# The config's DATA_TAG selects the dataset subdir; ``split`` picks which
# file inside it — one of {test, valid} (default: test). ``beam_width``
# defaults to 30. The checkpoint tag is ``${MODEL_TAG}_best`` (written by
# run_experiment.sh's Trainer).
#
# Override with env vars:
#   CKPT_TAG   — alternate checkpoint stem (default: ${MODEL_TAG}_best)
#   MAX_TEST   — cap on test examples (default: 500)

set -euo pipefail

CONFIG="${1:?usage: $0 configs/<name>.env [split] [beam_width]}"
SPLIT="${2:-test}"
BEAM_WIDTH="${3:-30}"
...
case "$SPLIT" in
    ...
    *)     FILENAME="${SPLIT}.txt" ;;   # literal filename stem
esac
...
CMD=(python3 "${REPO}/Training/mingpt/run.py" debug_beam
     ...
     --outputs_path "${PRED_DIR}/${CKPT_TAG}__${DATA_TAG}_${SPLIT}__beam${BEAM_WIDTH}.txt")
