...
# Round-3 cohort: d=256r3 / d=512r3 (fresh from-scratch runs). Monitor state
# lives in its own subdir so it doesn't commingle with r1/r2.
ROUND_LABEL="round3"
MONITOR_DIR="$REPO/data_storage/things_on_paper/monitor/${ROUND_LABEL}"   # gitignored
MODEL_DIR="$REPO/data_storage/things_on_paper/model/${ROUND_LABEL}"
MODELS_LIST="256r3 512r3"
mkdir -p "$MONITOR_DIR" "$MODEL_DIR"
cd "$REPO"
...
python3 - "$MODEL_DIR" "$REPO" "$CSV" "$ACSV" <<'PY' || true
...
python3 - "$MODEL_DIR" "$CSV" "$BEAM_MAX_TEST_FOR_BEST" <<'PY' || true
...
if os.path.isfile(csv_path):
    with open(csv_path) as f:
        for row in csv.DictReader(f):
            ...
            # Only treat a row as complete when the beam eval actually hit the
            # full test-set cap. Partial rows (slow early-training beam evals
            # that didn't finish within the 15-min monitor window) stay ineligible
            # for rotation so the next pass's back-fill can still update them.
            try:
                bt = int(float(row.get("beam7_total") or 0))
...
for D in $MODELS_LIST; do
    SRC="$MODEL_DIR/d2_arch_${D}_l6_best.pt"
    DST="$MODEL_DIR/d2_arch_${D}_l6_best_snapshot_${TS}.pt"
    if [[ ! -s "$SRC" ]]; then
    ...
    PREV_ITER=
    PREV_SIDECAR=$(ls -t "$MODEL_DIR/d2_arch_${D}_l6_best_snapshot_"*.jobs 2>/dev/null | head -1 || true)
...
# Eval scripts hardcode MODEL_DIR=things_on_paper/model/, so CKPT_TAG must
# include the round subdir prefix for them to locate the round3 checkpoints.
for D in $MODELS_LIST; do
    [[ -z "${SNAP[$D]:-}" ]] && continue
    BW="${BEAM_WIDTH_PER_D[$D]:-$BEAM_WIDTH_DEFAULT}"
    CKPT="${ROUND_LABEL}/${SNAP[$D]}"
    GREEDY_JOB[$D]=$(CKPT_TAG="$CKPT" MAX_TEST=200 sbatch --parsable \
        Training/things_on_paper/eval/run_greedy_eval.sh \
        Training/things_on_paper/configs/d2_arch_${D}_l6.env test)
    BEAM_JOB[$D]=$(CKPT_TAG="$CKPT" MAX_TEST="$BEAM_MAX_TEST" sbatch --parsable \
    ...
    ANALYSIS_JOB[$D]=$(CKPT_TAG="$CKPT" OUT_DIR="${ANALYSIS_OUT[$D]}" N_SAMPLES=200 LINE_IDX=0 \
        sbatch --parsable \
...
python3 "$REPO/dev/plot_snapshot_history_r3.py" || echo "plot regeneration failed (non-fatal)"
echo "=== pass complete ==="
